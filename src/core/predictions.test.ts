...
import { picksLeft, scores } from './selectors';
...
it('frees a pick slot once one of your predictions settles or is voided', () => {
  let state = as('Kevin');
  state = act(state, { type: 'pick', id: 'flights', choice: 'yes' }, NOW);
  state = act(state, { type: 'pick', id: 'fishing', choice: 'no' }, NOW);
  state = act(state, { type: 'pick', id: 'downtime', choice: 'yes' }, NOW);
  expect(picksLeft(state)).toBe(0);

  state = act(as('Deniz', state), { type: 'settle', id: 'flights', result: 'yes' }, NOW);
  state = act(as('Kevin', state), { type: 'voidPrediction', id: 'fishing' }, NOW);
  expect(picksLeft(state)).toBe(2);

  state = act(state, { type: 'pick', id: 'dinner', choice: 'yes' }, NOW);
  state = act(state, { type: 'pick', id: 'nap', choice: 'no' }, NOW);
  expect(picksLeft(state)).toBe(0);
  expect(() => act(state, { type: 'pick', id: 'story', choice: 'yes' }, NOW)).toThrow(/three/i);
  // The settled pick still scores.
  expect(pointsFor(scores(state), 'Kevin')).toBe(10);
});
