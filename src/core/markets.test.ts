...
import { MAX_LIVE_MARKETS, STARTING_CENTS, dollars, priceLabel, sharesFor, yesPrice } from './market';
...
const resolve = (state: State, outcome: 'yes' | 'no' | 'void', by: Attendee = 'Nick') =>
  act(as(by, state), { type: 'resolveMarket', id: idOf(state), outcome }, NOW);
...
it('opens a market for anyone playing, and says so in the feed', () => {
  ...
  expect(state.feed[0].text).toBe(`Jack opened a market: ${question}`);
...
it('buys shares at the quoted price, spends pretend dollars, and moves the price', () => {
  ...
  expect(state.feed[0].text).toBe(`Nate bought Yes for $10.00: ${question}`);
